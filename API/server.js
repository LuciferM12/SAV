...
app.post('/createNewProduct', upload.fields([
    ...
    try {
        ...
        client = await pool.connect();
        const idCatInt = parseInt(categoria, 10);
        // Iniciar la transacción
        await client.query('BEGIN');

        const imageTypes = ['prod'];
        const insertedImages = {};
        ...
        // Actualizar la tabla `negocio` con los IDs de las imágenes
        const updateQuery = `
        INSERT INTO productos (nomprod, precio, descripcion, id_imagen, id_categoria) VALUES ($1, $2, $3, $4, $5) RETURNING *
      `;
        const updateValues = [
            producto, precio, description, 
            insertedImages.prod,
            idCatInt
        ];
        await client.query(updateQuery, updateValues);
